...
MODEL_DIR=checkpoints/ted_aze_flores_aug${augID}/eng_aze/
COMET_DIR=comet
mkdir -p $MODEL_DIR

if [ "$#" -eq 2 ]; then
    FINETUNED_MODEL=checkpoints/ted_aze_flores_aug${augID_pre}/eng_aze/checkpoint_best.pt
else
    FINETUNED_MODEL=${PRETRAINED_DIR}/model.pt
fi
...
fairseq-train \
	...
	--finetune-from-model $FINETUNED_MODEL \
...
cat "$MODEL_DIR"/test_b5.score
cat "$MODEL_DIR"/valid_b5.score
