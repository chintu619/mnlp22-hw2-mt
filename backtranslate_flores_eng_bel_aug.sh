...
for i in ${!LANGS[*]}; do
  ...
  echo "Binarize the data... (eng-bel)"
...
MODEL_DIR=checkpoints/ted_bel_flores_aug${augID_pre}/eng_bel/
COMET_DIR=comet
mkdir -p $MODEL_DIR
...
fairseq-generate $BINARIZED_DATA \
    ...
    --beam 5  | grep ^H | cut -c 3- | sort -n | cut -f3- > "$RAW_DATA"/ted-train.synth-QED.bel
